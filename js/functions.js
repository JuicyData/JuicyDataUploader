...
document.getElementById('syncButton').onclick = () => {
  let eventID = document.getElementById('eventID').value;
  axios.get('http://localhost/apiv1/events/' + eventID + '/matches/').then(function(response) {
    console.log(response);
    scoringSystemSchedule = response.data.matches;
    tempSchedule = [];
    for (let i = 0; i < scoringSystemSchedule.length; i++) {
    ...
    for (let i = 0; i < tempSchedule.length; i++) {
      axios
        .get('http://localhost/apiv1/2019/events/' + eventID + '/matches/' + tempSchedule[i].matchNumber)
        .then(function(response) {
